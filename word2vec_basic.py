...
def read_data(filename,vocabulary_size):
    #bz_file = bz2.BZ2File(filename, 'rb')
    c = 0
    token = TweetTokenizer()
    data = []
    #print("Started Reading bz2.....")
    ...
    #        content = tokenizer.tokenize(line)
    #        content = [word for word in content if not regex.match(word)]
    #        data.extend(content)
    print("Started Reading user_text")
    ...
    print("finished user_text Reading.......")
    with open("train_labels.txt","r") as f:
        for line in f:
            ...
            #if c < 5:
            #    print(message)
    print("Finished train_labels")
    return data
  #Extract the first file enclosed in a zip file as a list of words
  #with zipfile.ZipFile(filename) as f:
    #data = tf.compat.as_str(f.read(f.namelist()[0])).split()
    #print("data now",data[:10])
  #return data
...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  @data_index: the index o
  f a word. You can access a word using data[data_index]
...
#    if sys.argv[1] == 'nce':
  ...
  filename = 'RC_2016-02.bz2'

  words = read_data(filename,vocabulary_size)
  print('Data size', len(words))


  ####################################################################################
  # Step 2: Build the dictionary an
  #d replace rare words with UNK token.
  ...
  print("unigram len: ", len(unigram_prob), vocabulary_size, len(count))
  data_index = 0
